...
pub trait EncodeCard: Sized {
    ...
    /// [`EncodingError::NoTypeSupport`].
    fn new_from_lits<CE: EncodeCard>(lits: Vec<Lit>, bound_type: BoundType) -> Result<CE, EncodingError> {
    ...
    /// Returns assumptions for enforcing an upper bound (sum of lits <= ub) or
    /// an error if the encoding does not support upper bounding. Make sure that
    ...
    /// nother with added to the encoding between the last call to
    /// [`EncodeCard::encode`] and this method, otherwise
    /// [`super::EncodingError::NotEncoded`] will be returned.
...
pub trait IncEncodeCard: EncodeCard {
    ...
    /// [`EncodingError::NoTypeSupport`].
    fn new_from_lits<ICE: IncEncodeCard>(lits: Vec<Lit>, bound_type: BoundType) -> Result<ICE, EncodingError> {
