...
use winnow::{
    ascii::{dec_int, dec_uint, line_ending, multispace0, multispace1, space1, till_line_ending},
    combinator::{alt, eof, opt, separated, seq, terminated},
    error::StrContext,
...
pub fn parse_cnf<R, VM>(reader: R) -> Result<SatInstance<VM>, super::Error>
where
    R: BufRead,
    VM: ManageVars + Default,
{
    let header_data = Parser::<CnfHeader, _>::new(reader).forward_to_body()?;
    let mut inst = SatInstance::<VM>::default();
    inst.var_manager_mut()
        .increase_next_free(Var::new(header_data.n_vars));
    for data in header_data.body_parser {
        match data? {
            CnfData::Clause(clause) => inst.add_clause(clause),
            CnfData::Comment(_) => (),
        }
    }
    Ok(inst)
...
///
/// Parsing errors or [`io::Error`].
pub fn parse_wcnf<R, VM>(reader: R) -> Result<OptInstance<VM>, super::Error>
where
    R: BufRead,
    VM: ManageVars + Default,
{
    let header_data = Parser::<WcnfHeader, _>::new(reader).forward_to_body()?;
    ...
    match header_data {
        WcnfHeaderData::Pre22 {
            ...
            for data in body_parser {
                match data? {
                    WcnfData::HardClause(clause) => constrs.add_clause(clause),
                    WcnfData::SoftClause { weight, clause } => {
...
///
/// Parsing errors or [`io::Error`].
pub fn parse_mcnf<R, VM>(reader: R) -> Result<MultiOptInstance<VM>, super::Error>
where
    R: BufRead,
    VM: ManageVars + Default,
{
    let mut constrs = SatInstance::<VM>::default();
    let mut objs = vec![];
    for data in Parser::<Mcnf, _>::new(reader) {
        match data? {
            McnfData::HardClause(clause) => constrs.add_clause(clause),
            McnfData::SoftClause {
                obj_idx,
                weight,
                clause,
...
fn cnf_p_line(input: &mut &str) -> ModalResult<(u32, usize)> {
    seq! { _: 'p', _: space1, _: "cnf", _: space1, dec_uint, _: space1, dec_uint, _: multispace0 }
...
fn wcnf_p_line(input: &mut &str) -> ModalResult<(u32, usize, usize)> {
    seq! { _: 'p', _: space1, _: "wcnf", _: space1, dec_uint, _: space1, dec_uint, _: space1, dec_uint, _: multispace0 }
...
fn clause(input: &mut &str) -> ModalResult<Clause> {
    terminated(separated(0.., lit, multispace1), clause_ending)
...
pub fn parse_lit(input: &str) -> Result<Lit, ParsingError> {
    lit.parse(input)
        .map_err(|e| ParsingError::from_parse(e, input, 0))
}
...
fn clause_ending<'i>(input: &mut &'i str) -> ModalResult<&'i str> {
    alt((
        (multispace0, alt((line_ending, eof))).take(),
        (
            multispace0,
            '0',
            alt((multispace1, (multispace0, alt((line_ending, eof))).take())),
        )
            .take(),
    ))
...
mod tests {
    ...
    fn parse_cnf_body_pass() {
        ...
        let parser = Parser::<CnfBody, _>::new(Cursor::new(data));
    ...
    fn parse_cnf() {
        ...
        let parsed_inst = super::parse_cnf(Cursor::new(data)).unwrap();
    ...
    mod opt {
        ...
        use crate::{
            clause,
            instances::{ManageVars, Objective, OptInstance, SatInstance},
            ipasir_lit,
            types::Var,
        };
        ...
        fn parse_wcnf_pre22() {
            ...
            let parsed_inst = super::super::parse_wcnf(Cursor::new(data)).unwrap();
        ...
        fn write_parse_wcnf() {
            ...
            let parsed_inst = super::super::parse_wcnf(cursor).unwrap();
    ...
    mod multiopt {
        ...
        fn parse_mcnf_body_pass() {
            ...
            let parser = Parser::<Mcnf, _>::new(Cursor::new(data));
            let clauses = parser.collect::<Result<Vec<_>, _>>().unwrap();
        ...
        fn parse_mcnf() {
            ...
            let parsed_inst = super::super::parse_mcnf(Cursor::new(data)).unwrap();
        ...
        fn write_parse_mcnf() {
            ...
            let parsed_inst = super::super::parse_mcnf(&mut cursor).unwrap();
