...
mod types;
pub use types::{
    AbsConstraintId, Axiom, Conclusion, ConstraintId, Derivation, ObjectiveUpdate, Order,
    OrderOrOrigVar, OrderVar, OutputGuarantee, OutputType, ProblemType, ProofGoal, ProofGoalId,
    ProofOnlyVar, RedundantDerivation, Scope, ScopedSubproofElement, SubproofElement, Substitution,
};
    ...
    Writer: io::Write,
{
    ...
    fn write_subproof<V, C, PI>(&mut self, mut proof: std::iter::Peekable<PI>) -> io::Result<()>
    where
        V: VarLike,
        C: ConstraintLike<V>,
            ...
            for element in proof {
                let bump_ids = match element {
                    ...
                    ScopedSubproofElement::Scope(scope) => {
                        ...
                        writeln!(&mut self.writer, "  {END}{RULE_TERM}")?;
                        bump
                    }
                };
                self.next_id += bump_ids;
    ...
    /// If writing the proof fails.
    #[cfg(not(feature = "version2"))]
    pub fn start_checker_timer<S>(&mut self, name: S) -> io::Result<TimerHandle>
    where
        S: AsRef<str>,
    {
    ...
    /// If writing the proof fails.
    #[cfg(not(feature = "version2"))]
    pub fn end_checker_timer(&mut self, handle: TimerHandle) -> io::Result<()> {
        let name = handle.0;
    ...
    /// If writing the proof fails.
    #[cfg(not(feature = "version2"))]
    pub fn is_deleted<V, C>(&mut self, constr: &C) -> io::Result<()>
    where
        V: VarLike,
        C: ConstraintLike<V>,
    {
    ...
    /// If writing the proof fails.
    #[cfg(not(feature = "version2"))]
    pub fn fail_checking(&mut self) -> io::Result<()> {
