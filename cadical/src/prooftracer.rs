...
///
/// Rather than passing [`rustsat::types::Clause`] to the tracer, we use [`CaDiCaLClause`], which
...
pub trait TraceProof {
    ...
    /// Notify the observer that the solve call ends with status [`rustsat::solvers::SolverResult`]
...
impl CaDiCaLClause<'_> {
    ...
    /// Converts the clause into an owned [`rustsat::types::Clause`]
    ///
