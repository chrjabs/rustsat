...
impl sat::Input<CaDiCaLNewApi> for CaDiCaLState<Input> {
    ...
    fn reserve(&mut self, max_var: Var) -> rustsat::MightMemout<&Self> {
        handle_oom!(
            unsafe { ffi::ccadical_reserve(self.handle.0, max_var.to_ipasir()) },
            noanyhow
        );
        Ok(self)
    }
    ...
    fn solve(self) -> rustsat::MightMemout<sat::SolveResult<CaDiCaLNewApi>> {
        ...
        Ok(match res {
            0 => sat::SolveResult::Unknown(CaDiCaLState::<Unknown> {
                handle: self.handle,
                state: Unknown(),
            }),
            10 => sat::SolveResult::Sat(CaDiCaLState::<Sat> {
                handle: self.handle,
                state: Sat(),
            }),
            20 => sat::SolveResult::Unsat(CaDiCaLState::<Unsat> {
                handle: self.handle,
