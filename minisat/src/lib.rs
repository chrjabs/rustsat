...
/// Minisat in the initialization state
#[derive(Debug)]
pub struct Init();

/// Minisat in the input state
#[derive(Debug)]
pub struct Input();

/// Minisat in the sat state
#[derive(Debug)]
pub struct Sat();

/// Minisat in the unsat state
#[derive(Debug)]
pub struct Unsat();

/// Minisat in the unknown state
#[derive(Debug)]
pub struct Unknown();

/// Implements the basic incremental API for the core and the simp solver
macro_rules! impl_api {
    ($slvty:ty, $statety:ident, $guardty:ident, $init:ident, $reserve:ident, $add:ident, $solve:ident, $val:ident, $conflict:ident) => {
